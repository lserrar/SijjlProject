...
- ✅ **Stripe Phase B — Subscriptions avec engagement 12 mois** (Fév 2026 — handoff fork) :
  ...
  - ✅ **Webhook configuré en LIVE mode** : 8 events souscrits dans Dashboard Stripe LIVE → `checkout.session.completed`, `customer.subscription.created/updated/deleted`, `invoice.paid`, `invoice.payment_failed`, `invoice.upcoming`, `subscription_schedule.completed`. `STRIPE_WEBHOOK_SECRET` ajouté dans `backend/.env` (preview). Handler vérifie la signature, dispatche correctement les 8 events (testés via payload signé manuellement → tous renvoient 200 `handled:true`). Signature invalide → 400. Event inconnu → 200 `handled:false` (graceful).
  - ⚠️ **À FAIRE EN PROD** : ajouter `STRIPE_WEBHOOK_SECRET=whsec_A12mlMEUza3xOIgZHaat7Gq8uMAK38Sa` dans le `.env` du VPS Hostinger avant `docker compose up -d`.
...
- ✅ **Scripts : parser DOCX/PDF respectant les sauts de ligne et les titres** (Fév 2026) :
  ...
  - **Mappings** : cours-debuts-islam (Bouali, 4 eps), cours-andalus (Ghouirgate), cours-historiographie (Ghouirgate, Ibn Khaldūn biographie), cours-fiqh (Mahil, droit musulman 6 eps), cours-coran (Azaiez), cours-hadith (Chahdi), cours-sciences (Ben Miled · Ben Saâd), cours-art (Grandpierre, 4 eps), cours-traduction (Sebti), cours-al-ghazali (Sebti), cours-al-kindi (Sebti, 5 eps), cours-al-farabi (Sebti, 5 eps), cours-avicenne (Sebti, 5 eps), cours-falsafa-occident (Mechelloukh, 7 eps), cours-inclassables (Molino-Machetto, Ibn Khaldūn 2 eps), cours-falsafa-persan (Rizvi, Mullā Ṣadrā 3 eps), cours-philo-juive (Roux, Maïmonide unifié 2 parties).
  - **Validation** : logs `Migration v15d: editorial catalogue applied to 17 course(s)`. Screenshot Al-Andalus → titre, description multi-paragraphes avec puces, scholar « Mehdi Ghouirgate », layout pleine largeur OK.
