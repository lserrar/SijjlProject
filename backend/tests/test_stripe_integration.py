...
if not BASE_URL:
    BASE_URL = "https://islamic-learning-35.preview.emergentagent.com"
...
class TestCheckoutEndpoints:
    ...
    def test_checkout_create_monthly_plan(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/checkout/create",
            headers=self.headers,
            json={"plan_id": "monthly", "origin_url": "https://islamic-learning-35.preview.emergentagent.com"}
        )
        assert response.status_code == 200
        data = response.json()
        assert "url" in data
    ...
    def test_checkout_create_annual_plan(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/checkout/create",
            headers=self.headers,
            json={"plan_id": "annual", "origin_url": "https://islamic-learning-35.preview.emergentagent.com"}
        )
        assert response.status_code == 200
        data = response.json()
        assert "url" in data
